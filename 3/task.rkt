...
(define (draw locations xs name)
  (for/fold ([x 0] [y 0] [t 0]) ([item xs])
    (match-define (pregexp "(.)(.+)" (list _ dir (app string->number len))) item)
    (define (update-x i)
      (match dir
        ["R" (+ i 1)]
        ["L" (- (+ i 1))]
    ...
    (for ([i len])
      (hash-update! locations (list (+ x (update-x i)) (+ y (update-y i)))
                    (curry cons (list name (+ t i 1))) '()))
    (values (+ x (update-x (sub1 len)))
            (+ y (update-y (sub1 len)))
            (+ t len))))
...
(define-task task-2
  ...
  (for ([line (in-lines)] [i (in-naturals)])
    (draw locations (string-split line ",") i))
  (calc-min locations (λ (key val) (apply + (map second val)))))
